...
func RunJob(options *common.ENOptions) map[string][]map[string]string {
	...
	jobs := map[string]_interface.ENScan{
		...
		"tycapi": &tycapi.TycAPI{Options: options},
		"kc":     &kuaicha.KC{Options: options},
		"rb":     &riskbird.RB{Options: options},
	}
	...
	for _, jobType := range options.GetType {
		...
		if _, ok := jobs[jobType]; !ok {
			// 判断如果是插件模式就跳过
			if options.IsPlugins {
				continue
			}
		}
		go func(jobType string) {
			defer func() {
				if x := recover(); x != nil && !options.IsDebug {
			...
			// 如果是不是插件模式就正常跑
			if _, ok := apps[jobType]; ok && !options.IsPlugins {
				gologger.Info().Msgf("正在通过%s获取 ⌈%s⌋ 信息", jobType, options.KeyWord)
				app := apps[jobType]
				data := getAppByKeyWord(options.KeyWord, options.GetField, app)
				rdata = common.InfoToMap(data, app.GetENMap(), fmt.Sprintf("%s⌈%s⌋", jobType, options.KeyWord))
			} else {
